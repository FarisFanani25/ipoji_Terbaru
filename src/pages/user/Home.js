...
const Home = () => {
    ...
    const [testimonials] = useState([testimonialImg1, testimonialImg2, testimonialImg3]);
                ...
                <Row>
                    {serviceData.map((service, index) => (
                        <Col lg="3" md="4" sm="6" key={index} className="mb-4">
                            <div className="service-card" style={{ backgroundColor: service.bg }}>
    ...
    return (
        <Helmet title={"Home"}>
                ...
                <Container>
                    ...
                    <Row>
                        <Col md="4" className="info-item">
                            <img src={thumbIcon} alt="Rasa Lebih Enak" className="info-icon" />
                            <h3>Rasa Lebih Enak</h3>
                            <p>Beras Organik Eka Farm memiliki rasa yang pulen sehingga enak untuk dikonsumsi</p>
                        </Col>
                        <Col md="4" className="info-item">
                            <img src={qualityIcon} alt="Kontrol Kualitas" className="info-icon" />
                            <h3>Kontrol Kualitas</h3>
                            <p>Kami selalu melakukan kontrol kualitas untuk memastikan produk yang sampai di tangan Anda betul-betul organik dan sesuai SNI agar bermanfaat bagi kesehatan Anda sekeluarga</p>
                        </Col>
                        <Col md="4" className="info-item">
            ...
            <Row>
                <Col md="12">
                    <div className="info-header">
                        <h2 className="section-title">ANDA INGIN BELI BERAS ORGANIK IPOJI?</h2>
                        <div className="info-buttons">
                        ...
                        {products.map(produk => (
                            <div key={produk.id_produk} className="product-item">
                                <div className="product-card">
                                    <img src={`http://localhost:8080/gambar/${produk.gambar_produk}`} alt={produk.nama_produk} className="w-100" />
                                </div>
                            </div>
                        ))}
                    </div>
    ...
    <div className="testimonial-carousel-container">
        <button className="carousel-control left" onClick={() => scrollTestimonials('left')}>&lt;</button>
        <div className="testimonial-carousel" ref={testimonialCarouselRef}>
            {testimonials.map((testimonial, index) => (
                <div key={index} className="testimonial-item">
                    <div className="testimonial-card">
                        <img src={testimonial} alt={`Testimonial ${index + 1}`} className="w-100" />
                    </div>
                </div>
            ))}
        </div>
        <button className="carousel-control right" onClick={() => scrollTestimonials('right')}>&gt;</button>
    </div>
</section>
</Container>
